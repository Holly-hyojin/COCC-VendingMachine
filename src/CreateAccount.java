...
public class CreateAccount {
	...
	public static void newAccount (String title, String message) {
		...
		Button validateEmail= new Button(); 
		validateEmail.setText("Validate");
		validateEmail.setOnAction(e -> {
			if(emailText.getText().contains("@"))  //check user input is includes '@'
				getConfirmationBox();
			else {	
		...
		HBox mail= new HBox();
		mail.getChildren().addAll(email, emailText, validateEmail);
		mail.setAlignment(Pos.CENTER_LEFT);
		mail.setStyle("-fx-spacing: 7;");
		validateEmail.setStyle("-fx-background-color: linen;");
	...
	public static void checkAge() {
		...
		if (period.getYears() >= 18) {
			System.out.println("\n\n18 or over. You can sign up!");
			getConfirmationBox();
		} else {
			System.out.println("\n\nTo sign up, You need to be more then 18 years.");
			getErrorMessage();
		}
